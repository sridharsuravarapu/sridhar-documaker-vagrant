...
## added to copy the doc install content to the nodes.

#    if ( $sshpass == false ) {
#      exec { "copy doc zip ${domain}":
#        command => "scp -oStrictHostKeyChecking=no -oCheckHostIP=no ${user}@${adminListenAdr}:${path}/doc_${domain}.zip ${path}/doc_${domain}.zip",
#      }
#    } else {
#      exec { "copy doc zip ${domain}":
#        command => "sshpass -p ${userPassword} scp -oStrictHostKeyChecking=no -oCheckHostIP=no ${user}@${adminListenAdr}:${path}/doc_${domain}.zip ${path}/doc_${domain}.zip",
#      }
#    }
#     $unZipCommand = " "

#     exec { "unzip ${domain}":
#        command => "unzip -user_name=${wlsUser} -password=${password}",
#        require => [File[$domainPath],
#                    Exec[ "copy domain jar ${domain}"]],
#     }

## end of code to copy doc install
